...
## CONFIGURATION
BATCH_SIZE = 64
LOGS_PATH = "logs/test/1/"

LOAD_PARAMS = False
PARAM_PATH = "logs/test/1/"
...
#VALID_SET_SIZE = data.validation_set_size # not all datasets support this
...
def model(x):
  ...
  dense1 = dense(flat, 2048, is_training, tf.nn.relu, scope="dense1", dropout=True,
                 initializer=tf.random_normal_initializer(stddev=0.01))
  dense2 = dense(dense1, 2048, is_training, tf.nn.relu, scope="dense2", dropout=True,
                 initializer=tf.random_normal_initializer(stddev=0.01))
  dense6 = dense(dense2, NUMBER_OF_CLASSES, is_training, tf.nn.relu, scope="dense6", dropout=False,
                 initializer=tf.random_normal_initializer(stddev=0.01))
  return dense6
...
with tf.variable_scope("model"):
  ...
  input_image_batch = tf.placeholder(tf.float32, shape=[BATCH_SIZE, IMG_HEIGHT, IMG_WIDTH, NUM_CHANNELS], name="input_image_batch")
