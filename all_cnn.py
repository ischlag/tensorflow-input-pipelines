...
## CONFIGURATION
BATCH_SIZE = 64
LOGS_PATH = "/home/schlag/MyStuff/GrowNeurons/Logs/AllCNN/1/"

LOAD_PARAMS = False
PARAM_PATH = "/home/schlag/MyStuff/GrowNeurons/Logs/AllCNN/1/"
...
DECREASE_LEARNING_RATE_N_TIMES = 4
...
with tf.device('/cpu:0'):
  ...
  #validation_image_batch, validation_label_batch = data.build_validation_data_tensor()
...
def model(x):
  ...
  block1 = conv_block(x, 50, "block1")
  pool1 = tf.nn.max_pool(block1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")

  block2 = conv_block(pool1, 100, "block2")
  pool2 = tf.nn.max_pool(block2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")

  conv1 = conv2d(pool2, 100, "1conv",
                k_h=3, k_w=3,
                stride_h=1, stride_w=1,
                initializer=tf.random_normal_initializer(stddev=0.01),
                bias=True,
                padding='SAME')
  norm1 = batch_norm(conv1, 100, is_training, scope="1bn")
  relu1 = tf.nn.relu(norm1)

  conv2 = conv2d(relu1, 100, "2conv",
                k_h=1, k_w=1,
                stride_h=1, stride_w=1,
                initializer=tf.random_normal_initializer(stddev=0.01),
                bias=True,
                padding='SAME')
  norm2 = batch_norm(conv2, 100, is_training, scope="2bn")
  relu2 = tf.nn.relu(norm2)

  conv3 = conv2d(relu2, 10, "3conv",
                k_h=1, k_w=1,
                stride_h=1, stride_w=1,
                initializer=tf.random_normal_initializer(stddev=0.01),
                bias=True,
                padding='SAME')
  norm3 = batch_norm(conv3, 10, is_training, scope="3bn")
  relu3 = tf.nn.relu(norm3)

  res = tf.reduce_mean(tf.reshape(relu3, [BATCH_SIZE, 64, NUMBER_OF_CLASSES]), 1)
  return res
